...
document.getElementById("delete-folder-btn").onclick = async () => {
  ...
  // First, fetch folder contents to show warning
  try {
    const infoResponse = await fetch(`/files?path=${selectedFile}`);
    let fileCount = 0;
    let folderCount = 0;

    if (infoResponse.ok) {
      const items = await infoResponse.json();
      fileCount = items.filter(item => !item.is_dir).length;
      folderCount = items.filter(item => item.is_dir).length;
    }

    let warningMessage = `Are you sure you want to delete folder "${folderName}"?\n\n`;

    if (fileCount === 0 && folderCount === 0) {
      warningMessage += "This folder is empty.";
    } else {
      warningMessage += `⚠️ This will permanently delete:\n`;
      if (folderCount > 0) warningMessage += `   - ${folderCount} subfolder(s)\n`;
      if (fileCount > 0) warningMessage += `   - ${fileCount} file(s)\n`;
    }
    ...
    if (response.ok) {
      let msg = `✅ Folder "${folderName}" deleted successfully`;
      if (data.deleted_files || data.deleted_folders) {
        msg += `\n(${data.deleted_files} files and ${data.deleted_folders} folders removed)`;
      }
      alert(msg);
...
// Check if a file is hidden (OS-specific rules)
function isHiddenFile(path) {
  const parts = path.split('/');
  for (const part of parts) {
    // Unix/Linux/Mac: files starting with .
    if (part.startsWith('.') && part !== '.' && part !== '..') {
      return true;
    }
    // Windows: files starting with ~ or named Thumbs.db, desktop.ini, etc.
    if (part.startsWith('~') ||
        part === 'Thumbs.db' ||
        part === 'desktop.ini' ||
        part === '$INDEX_allocation') {
      return true;
    }
  }
  return false;
...
// Get hidden file examples for each OS
function getHiddenFileExamples() {
  return {
    windows: [
      'desktop.ini',
      'Thumbs.db',
...
folderInput.onchange = async () => {
  ...
  // Separate hidden and visible files
  const hiddenFiles = [];
  const visibleFiles = [];
  ...
  // If there are hidden files, ask user
  if (hiddenFiles.length > 0) {
    const examples = getHiddenFileExamples();

    const message = `Found ${hiddenFiles.length} hidden file(s) in the selected folder.\n\n` +
      `Hidden files detected:\n${hiddenFiles.slice(0, 10).join('\n')}${hiddenFiles.length > 10 ? '\n...' : ''}\n\n` +
      `Examples of hidden files by OS:\n` +
      `🪟 Windows: ${examples.windows.slice(0, 3).join(', ')}\n` +
      `🍎 macOS: ${examples.macos.slice(0, 3).join(', ')}\n` +
      `🐧 Linux: ${examples.linux.slice(0, 3).join(', ')}\n\n` +
      `Do you want to include hidden files in the upload?\n\n` +
      `Click OK to include hidden files.\nClick Cancel to skip them.`;

    const includeHidden = confirm(message);

    if (!includeHidden) {
      // User chose to skip hidden files
      await uploadFiles(visibleFiles, 'Folder upload cancelled.');
      return;
    }
  }

  // No hidden files or user chose to include them
  await uploadFiles(Array.from(files), 'No hidden files detected.');
};

// Helper function to upload files
async function uploadFiles(files, skipMessage) {
  if (files.length === 0) {
    alert(skipMessage);
    uploadFolderBtn.textContent = "📁 Add Folder";
  ...
  for (let file of files) {
    ...
    // e.g., "MyFolder/subfolder/file.txt" -> path = "MyFolder/subfolder"
    const relativePath = file.webkitRelativePath || file.name;
    const pathParts = relativePath.split('/');

    if (pathParts.length > 1) {
      // Has folder structure - combine currentPath with parent folders
      const parentFolders = pathParts.slice(0, -1).join('/');
      const fullPath = currentPath ? `${currentPath}/${parentFolders}` : parentFolders;
  ...
  // Show result
  if (successCount > 0) {
    alert(`✅ Successfully uploaded ${successCount} file(s) from folder${failCount > 0 ? `, ${failCount} failed` : ''}`);
    loadFiles(currentPath); // Refresh the file list
  } else {
    alert("❌ Upload failed");
  }
...
document.getElementById("mobile-upload-folder")?.addEventListener("click", () => {
  closeMenu();
